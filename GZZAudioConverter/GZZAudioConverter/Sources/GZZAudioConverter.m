...
void startConvert(ExtAudioConverterSettings *settings) {
    ...
    while (1) {
        ...
        CheckError(ExtAudioFileRead(settings->inputFile, &framesCount, &outputBufferList), "ExtAudioFileRead failed");
        if (framesCount==0) {
            printf("Done reading from input file\n");
            return;
        }
        CheckError(ExtAudioFileWrite(settings->outputFile, framesCount, &outputBufferList), "ExtAudioFileWrite failed");
    }
}
...
void startConvertMP3(ExtAudioConverterSettings *settings) {
    ...
    while (1) {
        ...
        memcpy(pcm_buffer, outputBufferList.mBuffers[0].mData, framesCount);
        if (framesCount == 0) {
            printf("Done reading from input file\n");
            // Add lame_encode_flush for end of file
            return;
        }
        // the 3rd parameter means number of samples per channel, not number of sample in pcm_buffer
        write = lame_encode_buffer_interleaved(lame, outputBufferList.mBuffers[0].mData, framesCount, mp3_buffer, 0);
        fwrite(mp3_buffer, 1, write, outputFile);
    }
}

@implementation GZZAudioConverter
...
- (BOOL)convert {
    ...
    CheckError(ExtAudioFileOpenURL((__bridge CFURLRef)sourceURL, &settings.inputFile), "ExtAudioFileOpenURL failed");
    ...
    if (self.outputFormatID == kAudioFormatLinearPCM) {
        ...
        CheckError(AudioFormatGetProperty(kAudioFormatProperty_FormatInfo, 0, NULL, &size, &settings.outputFormat), "AudioFormatGetProperty kAudioFormatProperty_FormatInfo failed");
    }
    ...
    if (settings.outputFormat.mFormatID!=kAudioFormatMPEGLayer3) {
        CheckError(ExtAudioFileCreateWithURL((__bridge CFURLRef)outputURL, self.outputFileType, &settings.outputFormat, NULL, kAudioFileFlags_EraseFile, &settings.outputFile), "Create output file failed, the output file type and output format pair may not match");
    }
    ...
    if (settings.outputFormat.mFormatID==kAudioFormatLinearPCM) {
        ...
        // set format flags for both OS X and iOS, for all versions
        settings.inputPCMFormat.mFormatFlags = kAudioFormatFlagIsPacked | kAudioFormatFlagIsSignedInteger;
        // check if size of SInt16 is always suitable
        ...
        // check if this is suitable for both interleaved/noninterleaved
    ...
    CheckError(ExtAudioFileSetProperty(settings.inputFile, kExtAudioFileProperty_ClientDataFormat, sizeof(settings.inputPCMFormat), &settings.inputPCMFormat), "Setting client data format of input file failed");
    ...
    if (settings.outputFormat.mFormatID != kAudioFormatMPEGLayer3) {
        CheckError(ExtAudioFileSetProperty(settings.outputFile, kExtAudioFileProperty_ClientDataFormat, sizeof(settings.inputPCMFormat), &settings.inputPCMFormat), "Setting client data format of output file failed");
    }
...
- (void)validateInput:(ExtAudioConverterSettings*)settigs {
    ...
    switch (self.outputFileType) {
        ...
        case kAudioFileAIFCType: {
            // kAudioFileAIFCType together with kAudioFormatMACE3/kAudioFormatMACE6/kAudioFormatQDesign2/kAudioFormatQUALCOMM pair failed
        ...
        case kAudioFileCAFType: {
            ...
            // not all foramt are supported, check them out
            valid = YES;
            break;
        }
